...
/// 前向传播：对每个输入张量计算自适应平均池化，生成固定尺寸的输出。
/// 步长和池化窗口大小由输入尺寸与输出尺寸的比例推导得出。
...
/// 解析运行时算子参数以创建 AdaptiveAveragePoolingLayer 实例。
/// 期望 "output_size" 参数为二维整数数组 [output_h, output_w]。
