...
net_infer::sftensor PreProcessImage(const cv::Mat& image, const int32_t input_h,
...
int main() {
  ...
  for (uint32_t i = 0; i < batch_size; ++i) {
    const std::string& image_path = "/workspace/neural_infer/NetInfer/imgs/bus.jpg";  // 可以放不同的图片
    image_paths.push_back(image_path);
  }
  const std::string& param_path = "/workspace/neural_infer/NetInfer/models/yolo/yolov5s_batch8.pnnx.param";
  const std::string& bin_path = "/workspace/neural_infer/NetInfer/models/yolo/yolov5s_batch8.pnnx.bin";
