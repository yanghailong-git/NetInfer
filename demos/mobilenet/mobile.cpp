...
int main(int argc, char* argv[]) {
  if (argc != 2) {
    printf("usage: ./mobilenet_test [image path]\n");
    exit(-1);
  }
  using namespace net_infer;
  ...
  */
  const std::string& param_path = "/workspace/neural_infer/NetInfer/models/mobilenet/mobilenet_v2.pnnx.param";
  const std::string& weight_path = "/workspace/neural_infer/NetInfer/models/mobilenet/mobilenet_v2.pnnx.bin";
